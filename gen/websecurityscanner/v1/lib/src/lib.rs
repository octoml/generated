...
pub mod schemas {
    ...
    impl ::std::convert::AsRef<str> for XssAttackVector {
        fn as_ref(&self) -> &str {
            self.as_str()
        }
    }
    impl ::std::str::FromStr for XssAttackVector {
        type Err = ();
        fn from_str(s: &str) -> ::std::result::Result<XssAttackVector, ()> {
            Ok(match s {
    ...
    impl ::google_field_selector::FieldSelector for XssAttackVector {
        fn fields() -> Vec<::google_field_selector::Field> {
            Vec::new()
        }
    }
